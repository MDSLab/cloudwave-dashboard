...
horizon.d3_line_chart = {
  ...
  LineChart: function(chart_module, html_element, settings){
    ...
    */
    ...
    //We use the timeout (if specified) to remove old and unanswered requests to mongo for realtime charts.
    //FROM
    //self.refresh = function(optional) {
    //TO
    self.refresh = function(optional, optional_timeout) {

      var self = this;
      if (optional == undefined) self.start_loading();
      //ADDED for the timeout
      optional_timeout != undefined ? rt_timeout = optional_timeout : rt_timeout = false;
    //CLOUDWAVE STOP
      horizon.ajax.queue({
        url: self.final_url,
  ...
  //Changed in order to manage the timeout for mongo requests
  //FROM
  /*
  refresh: function refresh_chart(html_element, settings, optional) {
    var chart = new this.LineChart(this, html_element, settings);
    chart.refresh(optional);
  },
  */
  //TO
  refresh: function refresh_chart(html_element, settings, optional, optional_timeout) {
    var chart = new this.LineChart(this, html_element, settings);
    chart.refresh(optional, optional_timeout);
  },
  ...
  bind_commands: function (selector, settings){
    ...
    //CLOUDWAVE START
    var select_box_realtime_selector = 'button[data-line-chart-command="button_realtime_plot"]';
    delegate_event_realtime_selector = function (selector, event_name, settings) {
	$("body").on(event_name, selector, function () {
		connect_forms_to_charts();
		var invoker = $(this);
		var form = invoker.parents("form").first();
		var renderer = form.find("input:hidden[name=renderer]").val();

		/* Common variables */
		var id = $(form.data("charts_selector")).attr("id");
		var line_number = id.substring(7,id.length);
		var legend = $(form.data("charts_selector")).parents("div").parents("div").find("div[id=legendst"+line_number+"]");

		//FIX date offset from UTC and allows to have a common th_start_time if multiple charts are present
		//-----------------------------------------------------------------------------------------------
		var data = new Date();
		form.find("input:hidden[name=offset_time]").val(-data.getTimezoneOffset());
		if(document.forms.length == 1){
			$("#global_th_start_time").val(data.toISOString());
			form.find("input:hidden[name=th_start_time]").val(data.toISOString());
		}
		else{
			if($("#global_th_start_time").val() == ""){
				$("#global_th_start_time").val(data.toISOString());
			}

			form.find("input:hidden[name=th_start_time]").val($("#global_th_start_time").val());
		}
		//-----------------------------------------------------------------------------------------------


		if(renderer == 'multi-with-event' || renderer == 'multi-with-event-and-window'){
			settings.renderer = 'multi';
			$(form.data("charts_selector")).parents("div").show();

			if( $(form.data("charts_selector")).is(':empty') ==false)
				$(form.data("charts_selector")).empty();

			$(form.data("charts_selector")).show();

			$(form.data("charts_selector")).each(
				function event_refresh() {
					self.refresh(this, settings, 'abc');
					if( legend.is(':hidden')==true )
						legend.show();
				}
			);
		}
		...
		else if (renderer == 'line' || renderer == 'realtime-with-events'){
			if(renderer =='realtime-with-events') settings.renderer = 'multi';

			if(form.data("thread") != null){
				clearInterval(form.data("thread"));
				form.data("thread", null);
			}
			else{
				var SelectedOptions = form.find("select[id^=meterst]");
				if(SelectedOptions.val() == null){
					alert("Select at least a VM among the ones listed above!");
				}
				else if(SelectedOptions.val() != null){
					var inputInterval = form.find("input.interval")[0];
					$(form.data("charts_selector")).parents("div").show();
					...
					form.data("thread", setInterval(
						function(){
							$(form.data("charts_selector")).each(
								function rt_refresh() {
									//If the answer from mongo is delayed (twice the selected interval on the browser)
									//we skip (and remove) the older requests and go ahead with the more recent ones
									//which share the same thread start time.
									//FROM
									//self.refresh(this, settings, 'abc');
									//TO
									rt_timeout = $(inputInterval).attr("value") *2;
									self.refresh(this, settings, 'abc', rt_timeout);

									if( legend.is(':hidden')==true )
										legend.show();
								}
							);
						}, $(inputInterval).attr("value")
					));
				}
			}
		}
    ...
    bind_select_box_realtime_change = function (settings) {
	delegate_event_realtime_selector(select_box_realtime_selector, "click", settings);
    };
    bind_select_box_realtime_change(settings);
    //CLOUDWAVE STOP
