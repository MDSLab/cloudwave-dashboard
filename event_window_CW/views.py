...
from openstack_dashboard.dashboards.admin.event_window_CW import tabs as \
    metering_tabs
...
class IndexView(tabs.TabbedTableView):
    ...
    template_name = 'admin/event_window_CW/index.html'
...
class SamplesView(TemplateView):
	...
	#LOG.debug('EVENT_TYPE: %s EVENT_TIME: %s OFFSET_TIME: %s', event_type, event_time, offset_time)

	if event_time :
		hours_offset = int( int(offset_time) / 60)
		event_time = event_time[:10]+' '+event_time[11:19]
	...
	event_type = request.GET.get('selected_event_type', None)
	...
	if vms_selected_list != None and vms_selected_list != "":
		...
		event_time = request.GET.get('events_timestamps', None)

		search_time = event_time[:10]+' '+event_time[11:19]
		tmp_time = datetime.strptime(search_time, '%Y-%m-%d %H:%M:%S')

		start_time = tmp_time - timedelta(seconds=half_window_size)
		end_time = tmp_time + timedelta(seconds=half_window_size)
		
		#LOG.debug('START: %s, STOP: %s EVENT_TIME: %s', start_time, end_time, event_time)
